...
\caption{\PtwoE{} connects an evaluation ecosystem to feedback acquisition,
evaluator evolution, and independent downstream assessment. Panel (a) illustrates
the four domain families; each domain retains its own evaluation standard.
Panel (b) maps evidence, mines disagreements, and forms structured human feedback.
Panel (c) uses \ERA{} to revise a six-component program. Panel (d) separates
alignment and selection (C1), refinement (C2), and planned reward training (C3).
The GPT Image 2 artifact vignettes are synthetic illustrations, not experimental
cases; arrows specify information flow, not measured improvements.}
\label{fig:overview}
\end{figure}
