...
IterEval 是完整框架，包含 disagreement-based preference mining 和 depth-first evaluator evolution，两个子方法不设缩写。研究范围涵盖结构化视觉产物、图像与 3D、文本生成和自动科研，不限定为 slide generation。历史来源、代码标识符与计划文件名保持原样。
...
- [paper.pdf](paper.pdf)：编译预览，正文 10 页；尚超出初投稿的 9 页限制。
- [main.tex](main.tex)：主入口，Overleaf 选择此文件。
- [sections/](sections/)：正文和附录，新增独立的偏好挖掘章节。
- [figures/](figures/)：12 张图，2 张正文图、10 张附录图；其中 2 张概念图、1 张预算对比占位图、9 张模拟定量图。
...
- [全文术语约定](.paper/terminology.md)：disagreement-based preference mining、IterEval、候选接受与方向继续探索等固定用词；正文、附录和图注共用。
- [references.bib](references.bib)：52 条文献记录，当前正文及附录引用 35 条，含作者指定的四篇 2026 年论文；来源见 [reference_sources.json](.paper/reference_sources.json)，出版处核验、全文支持范围及未决项见 [citation_audit.md](.paper/citation_audit.md) 和 [最新相关工作审查](.paper/related_work_audit_20260920.md)。

正文保留 Figure 1 总览、Figure 2 横版 C1 组图，以及 Table 1 对齐与选择主表。新版 Figure 2 位于第 9 页，沿用任务环、右上散点、下方四族柱图、Source 字体和湖绿色 IterEval；右上改为偏好一致性与 Best-of-4 选择的关系，下方覆盖全部 22 个任务。主表按 ID/OOD 分列，完整比较指标、judge、校准、微调、prompt 优化、程序搜索和 IterEval，单元格仍待测。正文为 10 页，Results/Conclusion 的结果解释仍为条件式；作者修改后的 Abstract/Introduction 中的收益表述仍未得到实证验证。
...
论证主线遵循计划 §§1–2.3：开放任务中的质量判断依赖语境和多维取舍，现有指标与 judges 仍可能偏离人类偏好。偏好给出判断结果，却没有完整指定判断新产物所需的标准与证据。我们研究利用偏好数据推动评价 agent 自进化，使反馈能够用于改变可执行的评价过程。人类标准是目标，进化的是程序对该标准的显式近似；误判原因定位是支撑机制，而不是论文的首要问题。摘要依次交代对齐困难、偏好到评价过程的转化缺口、反馈驱动的进化方法及跨任务验证目标；参数适配对照和数据来源协议放在正文论证。IterEval 以 direction（改进方向）为探索单位、完整 evaluation program 为选择单位；hypothesis 只指支撑该方向的可检验判断。这些方法定义不构成绩效证据。
...
当前编译、术语、论文记忆和模拟图表一致性检查通过；`make check` 的投稿结构检查仍报告正文超页及 17 条未使用的文献记录。命名修改不代表可以投稿或验证了方法效果。详见 [IterEval 命名核对](.paper/itereval_naming_audit_20260920.md)。
...
生成器固定种子为 `20260916`，不调用模型、不执行 IterEval、不读取 benchmark 结果。数值、区间和成本仅用于布局示例，不能用于预测效果、功效分析或方法排名。本轮生成环境为 Python 3.8.10、NumPy 1.24.4、Matplotlib 3.7.5、fontTools 4.57.0；相同环境可确定性再生成。隔离环境中的字体处理依赖也可能改变 PDF 字节，因此跨环境再生成后需重新审阅版面和清单。
...
`make check` 检查当前 TeX 文本的术语一致性、引用、标签、9 页限制、溢出、图像哈希与排印分辨率、22 领域、主张状态、图表标题和正文引用，以及模拟数据的配对、缺失分母、数值表、一致性、弃权和成本汇总。结构与算术检查不代替学术审读或实证验证。
...
本轮按三张参考图分别建立候选，在相同数据、尺度、字体和布局下比较。正文暂用 A，将五色完整用于五种方法：静态 judge 浅蓝 `#74A9C5`、静态工具种子薄荷 `#C2E5CF`、prompt 优化奶油黄 `#EDDDAB`、程序搜索浅桃 `#F2B8AE`、IterEval 玫瑰粉 `#DD7389`；两种纯指标参照用灰色。B 采用第二张图的蓝—橙序列，C 采用第三张图的青蓝—杏橙序列，也均使用完整的五色组合。色值来自图片中明确标注的 HEX，不靠 JPEG 像素估色，不额外混入紫色或铜色。原图只出现在本地来源对照页，不进入论文或自动提交，也不使用其数值和显著性标记。
