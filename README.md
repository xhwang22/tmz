...
- [全文术语约定](.paper/terminology.md)：Informative Preference Mining（IPM）、ERA、候选接受与方向继续探索等固定用词；正文、附录和图注共用。
...
论证主线遵循计划 §§1–2.3：开放任务中的质量判断依赖语境和多维取舍，现有指标与 judges 仍可能偏离人类偏好。偏好给出判断结果，却没有完整指定判断新产物所需的标准与证据。我们研究利用偏好数据推动评价 agent 自进化，使反馈能够用于改变可执行的评价过程。人类标准是目标，进化的是程序对该标准的显式近似；误判原因定位是支撑机制，而不是论文的首要问题。摘要依次交代对齐困难、偏好到评价过程的转化缺口、反馈驱动的进化方法及跨任务验证目标；参数适配对照和数据来源协议放在正文论证。ERA 以 direction（改进方向）为探索单位、完整 evaluation program 为选择单位；hypothesis 只指支撑该方向的可检验判断。这些方法定义不构成绩效证据。
